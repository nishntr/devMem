...
When devmem is installed via pip, the shell/ directory isn't available.
This module embeds the hook scripts as strings so `devmem init` can
write them to ~/.config/devmem/.
"""

ZSH_HOOK = r'''# DevMem shell hook — source this in ~/.zshrc
...
BASH_HOOK = r'''# DevMem shell hook — source this in ~/.bashrc
# Installed by: devmem init
...
FISH_HOOK = r'''# DevMem shell hook — source this in ~/.config/fish/config.fish
# Installed by: devmem init
